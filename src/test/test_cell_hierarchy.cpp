...
int main(int argc, char** argv) {
    ...
    QMap<QString, QSet<QString>> refs;  // cell -> cells it references
    QString currentCell;
    bool inStr = false;
    ...
    while (!ds.atEnd()) {
        ...
        else if (type == 0x1206 && !currentCell.isEmpty() && data > 0) { // SNAME
            QByteArray n(data, 0); ds.readRawData(n.data(), data);
            while (n.endsWith('\0')) n.chop(1);
            QString ref = QString::fromLatin1(n);
            refs[currentCell].insert(ref);
            referencedCells.insert(ref);
        }
        else if (data > 0) ds.skipRawData(data);
    }
    
    out << "\n=== Top-Level Cells (not referenced) ===\n";
    QSet<QString> top = allCells - referencedCells;
    out << "Count: " << top.size() << "\n";
    for (const QString& c : top) out << "  - " << c << "\n";
    
    out << "\n=== User-specified cells check ===\n";
    QStringList check = {"Res_bansui_1_ab_Flip_$1", "Singlebit_V8_2_GRID_ARRAY(1)$1", 
                         "TiNPad&Inpillar(3)", "topcell000111"};
    for (const QString& c : check) {
        bool exists = allCells.contains(c);
        bool isTop = top.contains(c);
        bool isRef = referencedCells.contains(c);
        out << c << ": exists=" << exists << " isTopLevel=" << isTop << " isReferenced=" << isRef << "\n";
    }
